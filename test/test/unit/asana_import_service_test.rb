...
class AsanaImportServiceTest < ActiveSupport::TestCase
  ...
  def test_project_name
    assert_equal 'Web Sprint Board', Project.first.name
  end

  def test_project_members
    assert_equal 7, Project.first.members.size
  end
  ...
  def test_issue_size
    assert_equal 17, Project.first.issues.size
  end

  def test_issue_assigner
    assert_equal 'nazi9999', Project.first.issues.second.assigned_to.login
  end
